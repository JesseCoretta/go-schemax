...
*/
func (r MatchingRule) Marshal(def any) error {
	...
	for k, v := range m {
		...
		case `DESC`, `NUMERICOID`:
			z := map[string]func(string) MatchingRule{
...
func (r MatchingRule) marshalBoolean(v any) {
	switch tv := v.(type) {
	case string:
		if eq(tv, `TRUE`) {
			r.SetObsolete()
...
func (r MatchingRule) marshalExt(key string, v any) {
	if hasPfx(key, `X-`) {
		switch tv := v.(type) {
		case string:
...
/*
XOrigin returns an instance of [MatchingRules] containing only definitions
...
*/
func (r MatchingRules) XOrigin(x string) (defs MatchingRules) {
	defs = NewMatchingRules()
	for i := 0; i < r.Len(); i++ {
		def := r.Index(i)
...
the principal identifier of a [MatchingRule] and the provided id.
